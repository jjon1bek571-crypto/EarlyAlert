  ...
  return (
    <div className="fixed inset-0 z-[300] bg-black/95 backdrop-blur-2xl flex items-center justify-center p-4 md:p-6">
      <div className="max-w-md w-full bg-[#020617] border border-white/10 rounded-[32px] md:rounded-[48px] p-8 md:p-12 text-center animate-fadeIn shadow-[0_0_100px_rgba(220,38,38,0.1)] overflow-y-auto max-h-[90vh]">
        ...
        <h2 className="text-2xl md:text-3xl font-[900] text-white mb-3 md:mb-4 tracking-tighter uppercase italic">{steps[step].title}</h2>
        <p className="text-slate-400 font-bold mb-8 md:mb-12 leading-relaxed uppercase text-[10px] md:text-[12px] tracking-widest px-2 md:px-4">{steps[step].desc}</p>

        <div className="flex gap-2 md:gap-3 justify-center mb-8 md:mb-12">
          {steps.map((_, i) => (
        ...
        <div className="space-y-3 md:space-y-4">
          <button 
            ...
            className="w-full py-5 md:py-7 bg-white text-slate-950 rounded-[24px] md:rounded-[28px] font-black text-[10px] md:text-xs uppercase tracking-[0.2em] md:tracking-[0.3em] shadow-2xl hover:scale-105 active:scale-95 transition-all"
          >
          ...
          {step === steps.length - 1 && onReadManual && (
            <button 
              ...
              className="w-full py-3 md:py-4 text-slate-500 font-black text-[9px] md:text-[10px] uppercase tracking-widest hover:text-white transition-colors"
            >
              {t.readManual}
            </button>
          )}
